...
- vue.js는 `<transition></transition>`, 래퍼컴포넌트 wrapper component를 간단하 트랜지션을 적용하는 방법, 프로그래밍 방식의 동적 상태 전환 기능도 제공합니다.

## **13.1 css 트랜지션 기초**
- css트랜지션은 css 속성값을 서서히 변경해나가는 방법으로 트랜지션을 적용

    - 예제13-1은 transition 스타일 속성을 이용해 a 에서 b지점으로 서서히 이동하게됨<br>단순한 html과 css 조합으로 작성된 예제 13-01 transition 스타일 속성이나 이용하면 두 값 사이를 일정한 속도로 서서히 변환시킴 애니메이션 효과를 일으킴
